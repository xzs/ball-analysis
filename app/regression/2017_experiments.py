...
OUT_FOR_SEASON = ['Brandan Wright','DeMarcus Cousins', 'Dion Waiters']
...
OLD_OUT_LIST = {
    ...
    'OKC': ['Russell Westbrook','Andre Roberson'],
    'ORL': [],
    'PHI': [],
    'PHX': [],
    'POR': [],
...
def get_last_n_game_dk(player_name, n):
    ...
    dk_game_obj = []
        ...
        # 'diff': []
    # }
    last_n_games = sqlfetch.execute_query(sqlfetch.player_last_game(player_name, n, True))
...
# get_last_n_game_dk('Justin Anderson', 1)
...
def get_possible_lineups(team, oppo):
    ...
    out_from_team = OUT_PLAYERS + TRADED_LIST[sql_team] + OUT_FOR_SEASON
            ...
            if len(player_on_obj['lineups']) > 0:
                ...
                for usg_player in on_players:
                    ...
                    if sql_player not in LAST_PLAYER_SUMMARY_OBJ \
                        ...
                        and sql_player not in OUT_PLAYERS:
    ...
    with open('../scrape/csv/'+filename+'.csv',) as csv_file:
        try:
            ...
            for (team, oppo) in TEAMS_PLAYING.iteritems():
                ...
                if len(DIFF_OUTS[sql_team]['added']) > 1 and len(DIFF_OUTS[sql_oppo]['added']) > 1:
            ...
            # be great if we can do this by position
            ...
            for team, stats in team_summary.iteritems():
                ...
                out_from_team = OUT_PLAYERS + TRADED_LIST[team] + OUT_FOR_SEASON
                ...
                print '{team} - Days Rested: {days}, Out players: {players}'.format(\
                    team=team, days=stats['days_rested'], players=stats['out_players'])
                print '\n'
                print 'Team Depth'
                ...
                for player_name in ALL_TEAM_PLAYERS[dk_team]['all_players']:
                    ...
                    if sql_name not in OBVIOUS_OUTS['by_team'][team] \
                        ...
                        if player_name in DK_MONEY_OBJ:
                            ...
                            chalk_obj[sql_name] = {
                                ...
                                'value_goal': LINEUP_BASED_PROJ[sql_name]['value_goal'] if sql_name in LINEUP_BASED_PROJ else DK_MONEY_OBJ[player_name]['fp_needed'],
                                'avg_fp': float(DK_MONEY_OBJ[player_name]['fp_avg']),
                ...
                # pp.pprint(out_from_team)
                for idx, obj in enumerate(chalk_obj_sorted):
                    player_name = obj[0]
                    chalk_stats = obj[1]
                    ...
                    if player_name not in out_from_team:
                        ...
                        if np.isnan(chalk_stats['past_dkm']):
                            ...
                            # (-500) 4.73 @ 8300  -  Kyrie Irving  -  13.8pb (36.0 / 49.8)  -  63.24% (WAS, Last Match Up DKM: 0.97)
                            # favorable based on past
                            ...
                            if  avg_val >= 2 and \
                                ...
                                player_name in last_game_starters or \
                                ...
                                (player_name in last_n_playtime and last_n_playtime[player_name]['minutes_played'] >= 22) or \
                                (player_name in last_one_playtime and last_one_playtime[player_name]['minutes_played'] >= 22)):
                ...
                print 'Team CHALK bonus = {bonus}'.format(bonus=team_chalk_bonus)
            ...
            for position, position_players in FAVOR_OBJ.iteritems():
                ...
                for obj in chalk_obj_sorted:
                    ...
                    for idx, game in enumerate(last_n_game_dk):
                        ...
                        if idx in bypass_game:
                            ...
                            if abs(game['diff']) >= 12 and player_info['last_starter'] == '' and news_scraper.two_decimals(float(game['dk_points'])/(0.001*float(player_info['salary']))) >= 5:
                                if player_name in last_one_playtime:
                                    print '[O - {idx}] ({diff}) {player_name} {min} - U:{usg} - R: {dk}'.format(diff=game['diff'], min=game['min'], idx=idx, player_name=player_name, dk=game['dk_points'], usg=float(game['usg'])*100)
                                continue
                            else:
                                filtered_last_n_dk.append(game['dk_points'])

                            # game_str = str(game['dk_points']) + ', '
                            # last_n_game_dk_str += game_str
                    ...
                    csv_position[position][player_name] = {
                        ...
                        'exposure': default_exposure if default_exposure >= 0 else 0
                    }
                    ...
                    print '({lineup_rank}{lineup_add})({proj_rank})[r:{risk_count}] +[{added_injury_str}] -[{removed_injury_str}] {last_starter}{additional_stars}{added_risk} ({avg_val}/{avg_val_n}) @ {salary} (N: {min_need}/{min_need_max}) {player} ({proj_min} | {proj} ({proj_val}) | {proj_max} ({proj_val_max}) / {proj_start} ({proj_val_start}) | {proj_start_max} ({proj_val_start_max})) - U: {last_n_game_usg_str} - R: {last_n_game_dk_str} - ({oppo}({oppo_def_rank}), L: {dkm}, V: {vegas_diff} ({ou_score}) / {vegas_reg_weighted_score})'.format(\
                        ...
                        last_n_game_usg_str=last_n_game_usg_str, last_n_game_dk_str=last_n_game_dk_str, \
                ...
                print 'Whos Guarding Him?'
                print 'Whos more consistent in the good matchup'

                # get the guys who have tht highest medians > 5 sort by that

                print 'Who has conditions attached to them in the matchup'
            ...
            for position, position_players in csv_position.iteritems():
                ...
                position_players_sorted = sorted(position_players.iteritems(), key=lambda x: (x[1]['usg'], x[1]['dkm']), reverse=True)
                pos_idx = 0
                for obj in position_players_sorted:
                    ...
                    print '[r:{risk_count}], {projected_value}, ({lineup_rank})({proj_rank}), {usg}, {player_name}, {projection}, {exposure}, {must_play}'.format(player_name=player_name, \
                        proj_rank=values['rank'], projected_value=values['projected_value'], lineup_rank=values['lineup_rank'], usg=values['usg'] ,risk_count=values['risk_count'], \
                        projection=values['projection'], exposure=values['exposure'], must_play=must_play)
                print '\n'
            ...
            csv_obj_sorted = sorted(csv_obj.iteritems(), key=lambda x: (x[1]['exposure'], x[1]['usg'], x[1]['dkm']), reverse=True)
            for obj in csv_obj_sorted:
                ...
                if (values['lineup_rank'] == 1 or values['rank'] == 1 or values['position_rank'] == 1):
                    must_play = 'MUST PLAY '
                print '{must_play}({lineup_rank})({proj_rank})({pos_rank}), [r:{risk_count}], {usg}, {player_name}, {projection}, {exposure}'.format(player_name=player_name, \
                    proj_rank=values['rank'], lineup_rank=values['lineup_rank'], pos_rank=POSITION_RANK[player_name], usg=values['usg'] ,risk_count=values['risk_count'], \
                    projection=values['projection'], exposure=values['exposure'], must_play=must_play)
...
# news_scraper.create_player_salary_log('2018-10-16', '')

# init_out_players()
# init_player_advanced('2018-04-19')
# init_team_sportvu_fga('2018-04-19')
# get_dk_money_obj('2018-04-19', '')

# get total number of available players?

# out_from_team = OUT_LIST['OKC'] + TRADED_LIST['OKC'] + OUT_FOR_SEASON
# print process_pre_post_injury_depth_chart('OKC', out_from_team)
